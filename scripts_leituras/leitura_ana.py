...
# Média de uso de cpu da última hora
media = 0
total = 0
media_ram_hora = 0.0
tempo_atual =  datetime.now()
hora_atual =  tempo_atual.strftime("%H")
ultima_hora = (int(hora_atual) - 1)
pico_cpu = cpu[0]
user_pico = user[0]
...
for i in range(len(timestamp)):
    ...
    if (timestamp[i].hour == ultima_hora) :
        media += ram[i]
        total += 1

if not(media == 0.0):
    media_hora = media / total
...
for i in range(len(timestamp)):
    ...
    if (timestamp[i].hour == ultima_hora and pico_cpu <= cpu[i] ) :
        user_pico = user[i - 1]
        pico_cpu = cpu[i]
...
hatual_em_segundos = int(hora_atual) * 3600
...
for i in range(len(timestamp)):
    ...
    if (timestamp_s >= restricao_tempo):
        media_disco += disco[i]
        qtd_dados += 1
...
-- RAM --
Média de uso ({ultima_hora}h): {media_ram_hora:.2f}

-- Disco --
Média de uso (últimos {int(param_seg / 60)}m): {media_disco}
""")